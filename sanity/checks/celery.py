...
class TestCelery(unittest.TestCase):
    ...
    def test_4_source_code_of_celery_workers(self):
       ...
       for workername in self.stats.iterkeys():
           if self.stats[workername]['code_path']:
               worker_code_paths.append(self.stats[workername]['code_path'])
       ...
       for worker_code_path in worker_code_paths_distinct:
           ...
           logging.debug("Comparing the code in the current directory: %s with a SHA of: %s against the code that the celery workers are running: %s with a SHA of %s" % (os.getcwd(), shasum_for_current_directory, worker_code_path, shasum_for_celery_worker_code_path))
